...
# dictionary to save the line where the function ends
function_scope = {

}

# variable holds the current function we are in
current_function = None
line_number = -1


def update_current_funtion(line):
    global current_function
    if line[0] == "return":
        if not in_a_function_scope():
            current_function = None
    else:
        current_function = line[1]


def in_a_function_scope():
    global function_scope
    global current_function
    global line_number
    if current_function is None:
        return False
    elif function_scope[current_function] < line_number:
        return False
    else:
        return True
...
def push_static_index(line):
    global input_file
    assembly_commands = [
        "@" + input_file.name + "." + line[2] + "\n",
        "D=M\n",
        "@SP\n",
        "A=M\n",
        "M=D\n",
    ...
    write_assembly_to_output_file(assembly_commands)
...
def convert_push_command(line):
    ...
    if segment == "that" or segment == "this" or segment == "argument" or segment == "local":
...
def convert_label_command(line):
    assembly_label = ["(" + line[1] + ")\n"]
    if in_a_function_scope():
        assembly_label[0] = "(" + current_function + "$" + line[1] + ")\n"
    write_assembly_to_output_file(assembly_label)
...
def convert_goto_command(line):
    ...
    if in_a_function_scope():
        assembly_goto[0] = "@" + current_function + "$" + line[1] + "\n"
    write_assembly_to_output_file(assembly_goto)


def convert_if_goto_command(line):
    assembly_if_goto = [
        "@SP\n",
        "A=M\n",
        "D=M\n",
    ...
    if in_a_function_scope():
        assembly_if_goto[4] = "@" + current_function + "$" + line[1] + "\n"
    write_assembly_to_output_file(assembly_if_goto)


def convert_function_command(line):
    assembly_function_command = [
        "(" + line[1] + ")\n",
    ]
    if int(line[2]) == 0:
        write_assembly_to_output_file(assembly_function_command)
        return
    count = 0
    while count < int(line[2]):
        assembly_function_command.append("@SP\n")
        assembly_function_command.append("D=M\n")
        assembly_function_command.append("@" + str(count) + "\n")
        assembly_function_command.append("A=D+A\n")
        assembly_function_command.append("M=0\n")

        count += 1
    assembly_function_command.append("@" + line[2] + "\n")
    assembly_function_command.append("D=A\n")
    assembly_function_command.append("@SP\n")
    assembly_function_command.append("M=M+D\n")
    write_assembly_to_output_file(assembly_function_command)


def increase_SP_by_one():
    assembly_commands = [
        "@SP\n",
        "M=M+1\n",
    ]
...
def push_return_address(function_name):
    global current_function
    ...
    if in_a_function_scope():
        assembly_commands[0] = "@" + current_function + "-" + function_name + "-return\n"
        return_name = current_function + "-" + function_name + "-return"
    write_assembly_to_output_file(assembly_commands)
    push_D_register()
    increase_SP_by_one()
    return return_name
...
def convert_call_command(line):
    return_address = push_return_address(line[1])
    push_LCL()
    push_ARG()
    push_THIS()
    push_THAT()
    reposition_ARG(line[2])
    reposition_LCL()
    # goto function and (return-address)
    assembly_commands = [
        "@" + line[1] + "\n",
        "0;JMP\n",
        "(" + return_address + ")\n"
...
def determine_command(line):
    ...
    if line[0] == 'push':
        ...
        convert_label_command(line)
    elif line[0] == "goto":
        convert_goto_command(line)
    elif line[0] == "if-goto":
        convert_if_goto_command(line)
    elif line[0] == "call":
        convert_call_command(line)
    elif line[0] == "function":
        convert_function_command(line)
        update_current_funtion(line)
    elif line[0] == "return":
        update_current_funtion(line)
        convert_return_command(line)
    else:
...
def update_dict_function_scope(cleaned_line):
    global line_number;
    global function_scope;
    global current_function
    line_number += 1
    if cleaned_line[0] == "function" and cleaned_line[1] not in function_scope:
        current_function = cleaned_line[1]
        function_scope[cleaned_line[1]] = 9999999999
    elif cleaned_line[0] == "return":
        function_scope[current_function] = line_number


def dict_function_scope_init():
    global input_file;
    global line_number;
    global current_function
    for line in input_file:
        ...
        if line_split == [] or line_split[0] == "//":
            ...
            cleaned_line = clean(line_split)
            update_dict_function_scope(cleaned_line)
    line_number = -1;
    current_function = None


# open all the vm files and pass them to the parse functîon
...
def open_all_input_files(list_of_vm_files):
    open_output_file()  # e.g. StackTest.asm
    if "Sys.vm" in list_of_vm_files:
        list_of_vm_files.remove("Sys.vm")
        list_of_vm_files.insert(0, "Sys.vm")
    for file in list_of_vm_files:
        open_input_file(file)  # e.g. StackTest.txt
        dict_function_scope_init()
        global input_file;
        input_file.close();
        input_file = open(file, "r")
        parse_input_file()
    close_files()  # wd zrugwechsle maybe?


def change_working_dir_to_files_location():
    path = input("Enter Folder Path where your file(s) are located: ")
    try:
        os.chdir(path)
    except FileNotFoundError:
        print("The entered File or Directory does not exists.")
        change_working_dir_to_files_location()
    else:
        return path


def get_vm_files(all_files):
    vm_files = []
...
def open_all_available_vm_files():
    path = change_working_dir_to_files_location()
    all_files = os.listdir(path)
    return get_vm_files(all_files)


def main():
    list_of_vm_files = open_all_available_vm_files()
    open_all_input_files(list_of_vm_files)
